...
# 프로젝트 전체 완성률 < 95% >
  - 중복 tf 발생문제
  - 노드간 통신중 트래픽 과부하
두 문제를 해결하지 못해 완성률을 100% 달성하지 못함
<hr/>
...
- 24.12.14, 기본 소프트웨어 작업 30%, 김재환
  - realsense_camera노드의 image_raw를 yolov5노드에서 구독함
  - 카메라 앞의 물체가 사람이면 person이라는 토픽 데이터를, 사물이면 object라는 토픽데이터를 구독해서 전체 로봇을 제어하는 custom_control 노드 작성
<hr/>
...
- 24.12.13, 가제보 환경 제작 10%, 하정민
  - slam 맵 기반 가제보 맵 제작
- 24.12.16, 가제보 환경 시뮬레이션 10%, 장재희
  - 제작된 가제보 맵에서 충전위치로 이동하는 시퀀스 제작 및 테스트
<hr/>
